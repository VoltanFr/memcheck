...
{
    public RefreshImageUsages(TelemetryConfiguration telemetryConfiguration, MemCheckDbContext memCheckDbContext, MemCheckUserManager userManager, ILogger<RefreshImageUsages> logger)
        : base(telemetryConfiguration, memCheckDbContext, userManager, logger)
    {
    }
    [Function(nameof(RefreshImageUsages))]
    public async Task Run([TimerTrigger(Constants.Cron_RefreshImageUsages)] TimerInfo timer, FunctionContext context)
    {
