    ...
    public class CreateCardTests
    {
        ...
        public async Task WithAllData()
        {
            ...
            using (var dbContext = new MemCheckDbContext(testDB))
            {
                var request = new CreateCard.Request(
                    ...
                    versionDescription);
                cardGuid = await new CreateCard(FakeMemCheckTelemetryClient.InCallContext(dbContext)).RunAsync(request);
        ...
        public async Task MultipleImagesOnAdditionalSide()
        {
            ...
            using (var dbContext = new MemCheckDbContext(db))
                cardGuid = await new CreateCard(FakeMemCheckTelemetryClient.InCallContext(dbContext)).RunAsync(createRequest);
        ...
        public async Task SameImageUsedTwice()
        {
            ...
            using var dbContext = new MemCheckDbContext(db);
            await Assert.ThrowsExceptionAsync<RequestInputException>(async () => await new CreateCard(FakeMemCheckTelemetryClient.InCallContext(dbContext)).RunAsync(request));
        }
    }
}
