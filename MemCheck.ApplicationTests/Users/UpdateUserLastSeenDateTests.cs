...
public class UpdateUserLastSeenDateTests
{
    ...
    public async Task UserNotLoggedIn()
    {
        ...
        await Assert.ThrowsExactlyAsync<NonexistentUserException>(async () => await new UpdateUserLastSeenDate(dbContext.AsCallContext()).RunAsync(new UpdateUserLastSeenDate.Request(Guid.Empty)));
    }
    [TestMethod()]
    public async Task UserDoesNotExist()
    {
        var db = DbHelper.GetEmptyTestDB();
        using var dbContext = new MemCheckDbContext(db);
        await Assert.ThrowsExactlyAsync<NonexistentUserException>(async () => await new UpdateUserLastSeenDate(dbContext.AsCallContext()).RunAsync(new UpdateUserLastSeenDate.Request(RandomHelper.Guid())));
    }
    [TestMethod()]
    public async Task UserIsDeleted()
    {
        ...
        using (var dbContext = new MemCheckDbContext(db))
            await Assert.ThrowsExactlyAsync<NonexistentUserException>(async () => await new UpdateUserLastSeenDate(dbContext.AsCallContext()).RunAsync(new UpdateUserLastSeenDate.Request(userId)));
    }
    [TestMethod()]
