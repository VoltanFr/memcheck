...
public class GetAllUsersStatsTests
{
    ...
    public async Task UserNotLoggedIn()
    {
        ...
        await Assert.ThrowsExactlyAsync<InvalidOperationException>(async () => await new GetAllUsersStats(dbContext.AsCallContext()).RunAsync(new GetAllUsersStats.Request(Guid.Empty, 1, 0, "")));
    }
    [TestMethod()]
    public async Task UserDoesNotExist()
    {
        using var dbContext = new MemCheckDbContext(DbHelper.GetEmptyTestDB());
        await Assert.ThrowsExactlyAsync<InvalidOperationException>(async () => await new GetAllUsersStats(dbContext.AsCallContext()).RunAsync(new GetAllUsersStats.Request(Guid.NewGuid(), 1, 1, "")));
    }
    [TestMethod()]
    public async Task UserIsNotAdmin()
    {
        ...
        await Assert.ThrowsExactlyAsync<InvalidOperationException>(async () => await new GetAllUsersStats(dbContext.AsCallContext()).RunAsync(new GetAllUsersStats.Request(user, 1, 1, "")));
    }
    [TestMethod()]
    public async Task Page0()
    {
        ...
        var user = await UserHelper.CreateInDbAsync(db);
        using var dbContext = new MemCheckDbContext(db);
        await Assert.ThrowsExactlyAsync<PageIndexTooSmallException>(async () => await new GetAllUsersStats(dbContext.AsCallContext()).RunAsync(new GetAllUsersStats.Request(user, 1, 0, "")));
    }
    [TestMethod()]
    public async Task PageSize0()
    {
        ...
        var user = await UserHelper.CreateInDbAsync(db);
        using var dbContext = new MemCheckDbContext(db);
        await Assert.ThrowsExactlyAsync<PageSizeTooSmallException>(async () => await new GetAllUsersStats(dbContext.AsCallContext()).RunAsync(new GetAllUsersStats.Request(user, 0, 1, "")));
    }
    [TestMethod()]
    public async Task PageSizeTooBig()
    {
        ...
        var user = await UserHelper.CreateInDbAsync(db);
        using var dbContext = new MemCheckDbContext(db);
        await Assert.ThrowsExactlyAsync<PageSizeTooBigException>(async () => await new GetAllUsersStats(dbContext.AsCallContext(new TestRoleChecker(user))).RunAsync(new GetAllUsersStats.Request(user, GetAllUsersStats.Request.MaxPageSize + 1, 1, "")));
    }
    [TestMethod()]
