...
internal sealed class GetCardForEdit : ICmdLinePlugin
{
    ...
    public async Task RunAsync()
    {
        ...
        for (var i = 0; i < runCount; i++)
        {
            ...
            logger.LogInformation($"Got a card with {card.CountOfUserRatings} ratings, {card.Tags.Length} tags, {card.UsersOwningDeckIncluding.Length} users, {card.UsersWithVisibility} users with access in {oneRunChrono.Elapsed}");
