...
internal sealed class AddReferences : ICmdLinePlugin
{
    ...
    private static UpdateCard.Request RequestForReferencesChange(Card card, string references, Guid versionCreator, string versionDescription)
    {
        return new UpdateCard.Request(
            ...
            versionCreator,
            card.FrontSide,
            card.Images.Where(i => i.CardSide == ImageInCard.FrontSide).Select(i => i.ImageId).ToImmutableArray(),
            card.BackSide,
            card.Images.Where(i => i.CardSide == ImageInCard.BackSide).Select(i => i.ImageId).ToImmutableArray(),
            card.AdditionalInfo,
            card.Images.Where(i => i.CardSide == ImageInCard.AdditionalInfo).Select(i => i.ImageId).ToImmutableArray(),
            references,
